...
import adminProductGroupService from '@/services/admin/productGroup'
import type {
  ProductGroup,
  ProductGroupCreate,
  ProductGroupUpdate,
} from '@/types/admin/productGroup'

export const useProductGroupStore = defineStore('productGroup', {
  state: () => ({
    productGroups: [] as ProductGroup[],
    currentProductGroup: null as ProductGroup | null,
    loading: false,
  ...
  actions: {
    ...
    async fetchOne(id: number) {
        ...
        this.error = null
        try {
          const data = await adminProductGroupService.getOne(id)
          this.currentProductGroup = data
          return data 
        } catch (err: unknown) {
          this.error = getErrorMessage(err)
          console.error(err)
